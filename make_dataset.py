...
def parse_time_with_gpt(time_string):
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are a helpful assistant responding with JSON"},
            {"role": "user", "content": f"Extract the time and date from this string: {time_string} the format should be a json dict with {{'datetime': Year-Month-Date Hour:Minute Timezone}} Assume it is July 2024. Dont add any  ```json ```"}
        ]
    )
    try:
...
if __name__ == "__main__":
    ...
    for session_data in tqdm(json_data, desc='Processing Sessions'):
        ...
        embedding_content = f'Title: {session_data["name"]}; Abstract: {session_data["abstract"]}'
